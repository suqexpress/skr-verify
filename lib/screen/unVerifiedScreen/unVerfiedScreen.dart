    ...
    if (!_serviceEnabled) {
      ...
      if (locationPermission.isGranted) {
        ...
        if (!temp) {
          ...
          userLatLng =
              await Coordinates(_location.latitude, _location.longitude);
// location.hasPermission(locationPermission.);
  ...
  getCustomer() async {
    ...
    var response = await dio
        .get("https://erp.suqexpress.com/api/listcustomers")
    ...
    if (response.statusCode == 200) {
      ...
      for (var shop in response.data["data"]) {
        double dist = Geolocator.distanceBetween(
            ...
            double.parse(shop['long'].toString() == "null"
                ? 1.toString()
                : shop['long'].toString()),
            double.parse(shop['lat'].toString() == "null"
                ? 1.toString()
                : shop['lat'].toString()));
        ...
        child: ListView(
          ...
          children: <Widget>[
            ...
            ListTile(
              ...
              title: Text("Shops Not Found "),
              onTap: () {
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) => ShopNotFound()));
              },
            ),
          ],
        ),
